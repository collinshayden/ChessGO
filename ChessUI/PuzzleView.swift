...
struct board: View {
    ...
    var body: some View {
        ZStack {
                ...
                ForEach((0..<8).reversed(), id: \.self) {row in
                    HStack(spacing: 0) {
                        ...
                        ForEach(0..<8) {col in
                            ...
                            let squareColor = badMove ? PuzzleView.badColor : hint ? PuzzleView.hintColor : highlight ? PuzzleView.highlightColor : defaultSquareColor
            ...
            if logic.puzzleComplete {
                let k: Float = 100
                let startElo: Float = 1700
                var newElo = updateElo(userRating: startElo, userKFactor: k, puzzleRating: Int(logic.puzzle.rating) ?? 0, correct: true)
